...
int main(int argc, char **argv) {
  ...
  MCLoop loop(mcp);
  // Markov chain
  while (loop.step(0)) { // look up the value and by doing so increase the value (WHAT?)
    while (loop.step(1)) { // look up the value and by doing so increase the value (WHAT?)
      // make a move
      sys += mv.move();
